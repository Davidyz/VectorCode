...
async def get_query_result_files(
    ...
    from .reranker import NaiveReranker, CrossEncoderReranker

    # Try to use the reranker based on configuration
    try:
        if not configs.reranker:
            # Default to NaiveReranker
            reranker = NaiveReranker(configs)
        elif configs.reranker.startswith("cross-encoder/"):
            # Use CrossEncoder reranker
            reranker = CrossEncoderReranker(configs, query_chunks, configs.reranker)
        else:
            # Try to use a custom reranker from the new module system
            from vectorcode.rerankers import create_reranker
            reranker = create_reranker(configs.reranker, configs=configs, query_chunks=query_chunks)
    except Exception as e:
        # Fall back to NaiveReranker on any error
        print(f"Error loading reranker: {e}", file=sys.stderr)
